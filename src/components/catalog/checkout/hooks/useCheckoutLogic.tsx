  ...
  const handleCreateOrder = React.useCallback(async (onClose?: () => void) => {
    ...
    try {
      ...
      if (!customerData.name.trim()) {
        console.warn(' handleCreateOrder: Nome não informado');
        toast({
      ...
      if (!phoneNumbers || phoneNumbers.length < 10) {
        console.warn(' handleCreateOrder: Telefone inválido', { phone: customerData.phone, numbers: phoneNumbers });
        toast({
      ...
      if (cartItems.length === 0) {
        console.warn(' handleCreateOrder: Carrinho vazio');
        toast({
